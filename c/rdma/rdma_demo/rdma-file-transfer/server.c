...
static void on_pre_conn(struct rdma_cm_id *id)
{
  ...
  // printf("reg_mr before\n");
  TEST_Z(ctx->buffer_mr = ibv_reg_mr(rc_get_pd(), ctx->buffer, BUFFER_SIZE, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE)); /* 本地/远端写 */
  // printf("reg_mr after\n");
...
static void on_connection(struct rdma_cm_id *id)
{
  ...
  ctx->msg->data.mr.rkey = ctx->buffer_mr->rkey; // 将远端键rkey设置到msg上
