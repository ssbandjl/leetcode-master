...
static void write_remote(struct rdma_cm_id *id, uint32_t len)
{
  ...
  wr.imm_data = htonl(len); /* 立即数中存放内存长度(字节) */
  ...
  printf("IBV_WR_RDMA_WRITE_WITH_IMM\n");
...
static void send_file_name(struct rdma_cm_id *id)
{
  ...
  strcpy(ctx->buffer, ctx->file_name); // 拷贝文件名
...
static void on_completion(struct ibv_wc *wc)
{
  ...
  if (wc->opcode & IBV_WC_RECV) {
    if (ctx->msg->id == MSG_MR) {
      ...
      ctx->peer_rkey = ctx->msg->data.mr.rkey; // 拿到服务端的内存地址和键
...
int main(int argc, char **argv)
{
  ...
  rc_init(
    on_pre_conn,
    NULL, // on connect
    on_completion, // poll_cq_complete
    NULL); // on disconnect
