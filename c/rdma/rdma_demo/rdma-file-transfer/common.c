...
static char * get_wc_opcode_str(int opcode){
  switch (opcode) {
    case IBV_WC_SEND:
      return "IBV_WC_SEND";
...
void * poll_cq(void *ctx)
{
  ...
  while (1) {
    ...
    while (ibv_poll_cq(cq, 1, &wc)) {
      printf("wc.opcode:%d, %s\n", wc.opcode, get_wc_opcode_str(wc.opcode));
...
void rc_init(pre_conn_cb_fn pc, connect_cb_fn conn, completion_cb_fn comp /* poll_cq_complete */, disconnect_cb_fn disc)
{
  s_on_pre_conn_cb = pc;
