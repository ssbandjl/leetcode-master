...
void on_completion(struct ibv_wc *wc)
{
  ...
  if (wc->opcode & IBV_WC_RECV) {
    ...
    if (conn->recv_msg->type == MSG_MR) {
      memcpy(&conn->peer_mr, &conn->recv_msg->data.mr, sizeof(conn->peer_mr)); //将对端发来的数据拷贝到peer_mr
  ...
  if (conn->send_state == SS_MR_SENT && conn->recv_state == RS_MR_RECV) {
    ...
    sge.addr = (uintptr_t)conn->rdma_local_region;  // write, 将本地sge写到remote_addr
...
void send_mr(void *context)
{
  ...
  memcpy(&conn->send_msg->data.mr, conn->rdma_remote_mr, sizeof(struct ibv_mr));   // 传输remote_mr
